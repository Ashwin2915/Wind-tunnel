...
% If 'dE' exists, filter dataset to dE = 0 (for propOff)
if has_dE 
...
% If 'V' exists, separate dataset into V = 20 and V = 40 subsets 
if has_v 
    v20_filter = (abs(data_struct.V - 20) < 1);
    ...
    for i = 1:numel(field_names)
        ...
        field = field_names{i};
        data_V20.(field) = data_struct.(field)(v20_filter);
        data_V40.(field) = data_struct.(field)(v40_filter);
    end
end

% Define test points for interpolation
AoA_test = [0, 4, -2, -2, 4, 1, 3, 8];
AoS_test = [0, 10, 10, -3, -3, 3, 5, 0];
V_test = [20, 40];
dR_test = [0, 5, 10];
...
for i = 1:numel(field_names)
    ...
    if ~ismember(field, {'AoA', 'AoS', 'Vinf', 'V', 'dE', 'dR'})
        if has_dE
            interpolants_V20.(field) = scatteredInterpolant( ...
                data_V20.AoA, data_V20.AoS, data_V20.dR, data_V20.(field), 'linear', 'nearest');
            interpolants_V40.(field) = scatteredInterpolant( ...
                data_V40.AoA, data_V40.AoS, data_V40.dR, data_V40.(field), 'linear', 'nearest');
        elseif has_v
            interpolants_V20.(field) = scatteredInterpolant( ...
                data_V20.AoA, data_V20.AoS, data_V20.(field), 'linear', 'nearest');
            interpolants_V40.(field) = scatteredInterpolant( ...
    ...
    if ~ismember(field, {'AoA', 'AoS', 'Vinf', 'V', 'dE', 'dR'})
        if has_dE
            ...
            for j = 1:length(dR_test)
                ...
                results.(field)(:, :, 1, j) = interpolants_V20.(field)(AoA_test, AoS_test, dR_val);
                results.(field)(:, :, 2, j) = interpolants_V40.(field)(AoA_test, AoS_test, dR_val);
            end
        elseif has_v
            results.(field) = zeros(1, length(AoS_test), 2); % (AoA, AoS, V)
            results.(field)(:, :, 1) = interpolants_V20.(field)(AoA_test, AoS_test);
