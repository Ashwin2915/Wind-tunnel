...
for k = 1:numel(coeffNames)
    ...
    xA = modelOff.alpha0.AoA(:);
    yA = modelOff.alpha0.(nm)(:);

    xB = modelOff.beta0.AoS(:);
    yB = modelOff.beta0.(nm)(:);

    Cin = dataIn.(nm)(:);

    C_a0 = interp1(xA, yA, AoA, 'linear', 'extrap');
    C_0b = interp1(xB, yB, AoS, 'linear', 'extrap');

    C_a0 = C_a0(:);
    C_0b = C_0b(:);

    C_00_a = interp1(xA, yA, 0, 'linear', 'extrap');
    C_00_b = interp1(xB, yB, 0, 'linear', 'extrap');
    C_00 = 0.5 * (C_00_a + C_00_b);

    C_modeloff = C_a0 + C_0b - C_00;

    dataOut.(nm) = Cin - C_modeloff;
    ...
    disp(size(C_a0))
    disp(size(C_0b))
    disp(size(C_modeloff))
    disp(size(dataOut.(nm)))
end
end
