...
At = 0.40;      % tunnel test-section area [m^2]
alpha_up = 0.0; % tunnel upflow correction [deg]

V_model = struct();
V_model.rudder_0_block1     = 0.0;
V_model.rudder_0_block3     = 0.0;
V_model.rudder_0_block4     = 0.0;
V_model.rudder_m10_block6_7 = 0.0;
V_model.rudder_p5_block7b   = 0.0;
V_model.rudder_p10_block1   = 0.0;
V_model.rudder_p10_block5   = 0.0;
...
for i = 1:nCfg
    ...
    % ---------- 3) Wall corrections ----------
    dalpha_w = delta.(nm) .* CLu .* 57.3;
    dalpha_up_rad = deg2rad(alpha_up);

    AoA_corr = D0.AoA + alpha_up + dalpha_w;

    dCD_up = CLu .* dalpha_up_rad;
    dCD_w  = delta.(nm) .* CLu.^2;
    CDc = CDu + dCD_up + dCD_w;

    CLc = CLu;
    CYc = CYu;
    Clc = Clu;
    Cmc = Cmu - dCm_dCL.(nm) .* CLu;
    Cnc = Cnu;
